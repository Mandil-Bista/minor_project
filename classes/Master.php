...
Class Master extends DBConnection {
	...
	function update_request_status(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `selling_form` set `status` = '$status' where id = '{$id}' ");
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success"," Request status successfully updated.");
		}else{
			$resp['status'] ='failed';
			$resp['err'] =$this->conn->error;
		}
	...
	function accept_request(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `selling_form` set `paid` = '1' where id = '{$id}'");
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success"," Status successfully updated.");
		}else{
			$resp['status'] ='failed';
			$resp['err'] =$this->conn->error;
		}
